...
public class GameState implements Serializable {
	...
	final         String[]      SolarSystemName           = {"Acamar", "Adahn",
	                                                         ...
	                                                         "Cestus", "Cheron", "Courteney",
	                                                         // After Courteney Cox...
	                                                         "Daled", "Damast", "Davlos", "Deneb",
	                                                         "Deneva", "Devidia", "Draylon", "Drema",
	                                                         "Endor", "Esmee",
	                                                         ...
	                                                         "Malcoria", "Melina", "Mentar",
	                                                         // The Psilon home system in Master of Orion
	                                                         "Merik", "Mintaka", "Montor",
	                                                         // A city in Ultima III and Ultima VII part 2
	                                                         "Mordan", "Myrthe",
	                                                         // The name of my daughter (comment: Pieter Sproncks daughter)
	                                                         "Nelvana", "Nix",
	...
	public static int           CountDown                 = 3; // Countdown for reset of tradeitems.
	...
	public GameState(String NameCommander) {
		...
		while (i < GameState.MAXSOLARSYSTEM) {
			if (i < GameState.MAXWORMHOLE) {
				...
				SolarSystem[i].y =
					(((GameState.CLOSEDISTANCE / 2) - GetRandom(GameState.CLOSEDISTANCE)) + ((GameState.GALAXYHEIGHT * (i < 3 ?
					                                                                                                    1 :
					                                                                                                    3)) / 4));
				Wormhole[i] = i;
			} else {
	...
	public void GenerateOpponent(int Opp) {
		...
		while (j < Tries) {
			...
			while (Redo) {
				...
				if (Opp == POLICE && (ShipTypes.ShipTypes[i].police < 0 ||
            Politics.mPolitics[SolarSystem[WarpSystem].politics].strengthPolice + k < ShipTypes.ShipTypes[i].police))
					continue;

				if (Opp == PIRATE && (ShipTypes.ShipTypes[i].pirates < 0 ||
            Politics.mPolitics[SolarSystem[WarpSystem].politics].strengthPirates + k < ShipTypes.ShipTypes[i].pirates))
					continue;

				if (Opp == TRADER && (ShipTypes.ShipTypes[i].traders < 0 ||
            Politics.mPolitics[SolarSystem[WarpSystem].politics].strengthTraders + k < ShipTypes.ShipTypes[i].traders))
					continue;
